...
@login_required(login_url='/login/')
def index(request):
    remote_user_bind_hosts = models.RemoteUserBindHost.objects.filter(enabled=True)
...
def connect(request, user_bind_host_id):
    ...
    try:
        remote_user_bind_host = models.RemoteUserBindHost.objects.filter(enabled=True,id=user_bind_host_id)[0]
